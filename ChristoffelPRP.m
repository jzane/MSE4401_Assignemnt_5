function ChristMatrix = ChristoffelPRP(D, qDot)
...
for k = 1:3
    for j = 1:3
        for i = 1:3
            ...
            C_temp(i,j,k) = diff(D(k,j),q(i)) + diff(D(k,i),q(j)) - diff(D(i,j),q(k));
...
ChristMatrix(1:3,:) = C_temp(:,:,1)*qDot;
ChristMatrix(4:6,:) = C_temp(:,:,2)*qDot;
ChristMatrix(7:9,:) = C_temp(:,:,3)*qDot;
